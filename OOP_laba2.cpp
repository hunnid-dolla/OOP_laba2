...
int main() {
    ...
    while (true) {  // Бесконечный цикл для демонстрации различных операций
        cout << "\n=============================" << endl;
        cout << "МЕНЮ:" << endl;
        ...
        switch (choice) {  // В зависимости от выбора выполняется тот или иной блок кода
        case 1: {
            cout << "\n=============================" << endl;
            cout << "СТАТИЧЕСКИ СОЗДАВАЕМЫЕ ОБЪЕКТЫ" << endl;
            Point p1;  // Создание статического объекта точки
            Point p2(3, 4);  // Создание статического объекта точки с координатами (3, 4)
            Point p3 = p2;  // Копирование объекта p2 в p3
        ...
        case 2: {
            ...
            Point* p4 = new Point(5, 6);  // Создание динамического объекта Point
            p4->show();  // Вывод координат точки
            delete p4;  // Удаление динамически выделенной памяти
            break;
        }
        case 3: {
            cout << "\n=============================" << endl;
        ...
        case 4: {
            ...
            MyBase* baseObj = new MyBase("BaseObj");
            baseObj->hello();  // Вызов метода hello для baseObj
            delete baseObj;  // Удаление объекта
            break;
        }
        case 5: {
            cout << "\n=============================" << endl;
            cout << "ПОМЕЩЕНИЕ ОБЪЕКТОВ В ПЕРЕМЕННЫЕ РАЗЛИЧНЫХ ТИПОВ" << endl;
            MyBase* obj1 = new MyBase("BaseObj");
            obj1->hello();  // Вызов метода hello для obj1
            delete obj1;  // Удаление объекта

            MyBase* obj2 = new MyDeriv("DerivedObj", 42);  // Объект производного класса
            obj2->hello();  // Вызов метода hello для obj2
            delete obj2;  // Удаление объекта
            break;
        }
        case 6: {
            cout << "\n=============================" << endl;
            cout << "ОБЪЕКТЫ КЛАССОВ-НАСЛЕДНИКОВ" << endl;
            MyDeriv derivObj("DerivedObj", 100);
        ...
        case 7: {
            ...
            Container container;  // Создание контейнера, который содержит точку и круг
            break;
        }
        case 8: {
            cout << "\n=============================" << endl;
            cout << "КОМПОЗИЦИЯ ОБЪЕКТОВ: АТРИБУТОМ ЯВЛЯЕТСЯ УКАЗАТЕЛЬ НА ОБЪЕКТ ДРУГОГО КЛАССА" << endl;
            DynamicRectangle* drect = new DynamicRectangle();  // Создание динамического прямоугольника
            drect->print();  // Вывод информации о прямоугольнике
            delete drect;  // Удаление объекта
            break;
        }
        case 9: {
            cout << "\n=============================" << endl;
